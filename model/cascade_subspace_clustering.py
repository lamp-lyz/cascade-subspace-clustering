...
from keras.backend.tensorflow_backend import mean as tf_mean
import numpy as np
...
initial_centers = k_means.fit(encoded).cluster_centers_
...
x_train_norm = np.reshape(np.power(np.sum(np.power(x_train, 2), 1), 0.5), (x_train.shape[0], 1))
initial_centers_norm = np.reshape(np.power(np.sum(np.power(initial_centers, 2), 1), 0.5), (1, initial_centers.shape[0]))
norm_product = np.multiply(x_train_norm, initial_centers_norm)
...
pre_input = Input(shape=(encoding_dim,))
pre_layer = Dense(k_cluster, )


lossfunc = kullback_leibler_divergence(y_true, y_pred)

clustering_model = Model(pre_input)
